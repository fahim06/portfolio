...
function ProjectCard({project, isHighlighted = false}: { project: Project; isHighlighted?: boolean }) {
    return (
        <article
            className={`bg-background border rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 ${
                isHighlighted
            ...
            {isHighlighted && (
                <div
                    className="absolute top-4 right-4 bg-accent text-white text-xs font-semibold px-3 py-1 rounded-full z-10">
                    ⭐ Highlighted
                </div>
            )}
            {/* Header */}
          ...
          <span className="text-xs font-medium text-accent uppercase tracking-wider">
            {project.category}
                    ...
                    {project.featured && !isHighlighted && (
                        <span
                            className="text-xs font-medium text-foreground-muted bg-background-secondary px-2 py-1 rounded">
              Featured
            </span>
                    )}
                </div>
