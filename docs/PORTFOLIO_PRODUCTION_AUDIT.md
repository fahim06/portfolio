...
| Formatter (Prettier) | `npx prettier@3 --check .` (repo `.prettierrc`) | RAN, FAILED-CHECK — 72 files failed; root cause: config said `semi: false` while the codebase is written with semicolons. Config corrected to `semi: true` (matches reality), reducing failures to 40 (import wrapping/line width). Mass reformat deliberately NOT applied — owner has uncommitted work in `projects.js`/`personalInfo.js`; recommend one dedicated `prettier --write` commit once that lands |
| Type checker | `npx tsc --noEmit --allowJs --checkJs` over `src/` | RAN — ~60 findings, all inference artifacts of checking untyped JS JSX (destructured component props inferred as required, `key` props, CSS-module imports: 31× TS2307); **zero real type defects** found in logic. Not recommended to adopt without migrating to types |
| Automated a11y — axe (jsdom) | `jest-axe` in Vitest (`src/a11y.test.jsx`, runs in `npm test`) | PASSED — 0 violations across home (all lazy sections mounted), `/accessibility`, and 404. First run caught 2 real issues, both fixed: scroll-progress bar outside any landmark (moved into `<main>`), and brand button failing WCAG 2.5.3 Label-in-Name (aria-label now contains visible text). Note: color-contrast rules are "incomplete" under jsdom (no layout engine) — contrast covered by manual math instead |
| Automated a11y/perf/SEO — Lighthouse 12 (headless Chrome, production preview build) | `npx lighthouse@12 http://localhost:4173` | Accessibility **100**, SEO **100**, Best practices **96**, Performance **67**. Perf diagnosis: LCP 13.6s — the hero portrait is a 1.2MB PNG (2.0MB about PNG) served unprioritized/unoptimized; FCP 3.5s; TBT 0ms, CLS 0. Added `fetchpriority=high` preload (minor gain). Real fix needs next-gen image conversion — deferred: it touches `personalInfo.js` (owner has uncommitted edits there). `charset` sub-audit is a verified false positive (charset is first in `<head>` in source and dist) |
